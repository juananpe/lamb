<script>
	...
	function validateForm() {
		...
		if (!name.trim()) {
			...
			nameError = $_('knowledgeBases.createModal.nameTooLong', {
				default: 'Name must be less than 50 characters'
			});
			isValid = false;
		}
...
{#if isOpen}
	<div class="fixed inset-0 z-40 overflow-y-auto">
		...
		<div class="flex min-h-screen items-center justify-center p-4">
			<div
				class="relative mx-auto w-full max-w-lg rounded-lg bg-white p-6 shadow-xl"
			...
			>
				...
				<div class="mb-4">
					...
					<p class="mt-1 text-sm text-gray-500">
						{$_('knowledgeBases.createModal.description', {
				...
				<form onsubmit={handleSubmit} class="space-y-4">
					...
					<div>
						...
						<input
							...
							id="kb-name"
							bind:value={name}
							class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-[#2271b3] focus:ring-[#2271b3] focus:outline-none sm:text-sm {nameError
								? 'border-red-300 focus:border-red-500 focus:ring-red-500'
								: ''}"
							placeholder={$_('knowledgeBases.namePlaceholder', {
								default: 'Enter knowledge base name'
							})}
						/>
						{#if nameError}
					...
					<div>
						...
						<textarea
							...
							rows="3"
							class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-[#2271b3] focus:ring-[#2271b3] focus:outline-none sm:text-sm"
							placeholder={$_('knowledgeBases.descriptionPlaceholder', {
					...
					<div>
						<fieldset>
							...
							<div class="mt-2 space-y-2">
								<div class="flex items-center">
									<input
										...
										value="private"
										class="h-4 w-4 border-gray-300 text-[#2271b3] focus:ring-[#2271b3]"
									/>
							...
							<p class="mt-1 text-xs text-gray-500">
								{$_('knowledgeBases.accessControlHelp', {
									default:
										'Private knowledge bases are only accessible by you, while public ones can be accessed by other users.'
								})}
							</p>
						</fieldset>
					</div>
					...
					<div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
						<button
							type="submit"
							class="inline-flex w-full justify-center rounded-md border border-transparent bg-[#2271b3] px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-[#195a91] focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none sm:col-start-2 sm:text-sm"
						...
						<button
							type="button"
							onclick={close}
							class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:outline-none sm:col-start-1 sm:mt-0 sm:text-sm"
							disabled={isSubmitting}
						>
