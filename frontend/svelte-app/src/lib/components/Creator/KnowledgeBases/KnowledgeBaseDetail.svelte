<script>
	...
	async function fetchIngestionPlugins() {
		...
		try {
			...
			ingestionPluginsError =
				err instanceof Error ? err.message : 'Failed to load ingestion plugins';
			if (err instanceof Error && err.message.includes('server offline')) {
				serverOffline = true;
			}
		} finally {
	...
	function selectIngestionPlugin(index) {
		if (index >= 0 && index < ingestionPlugins.length) {
			...
			console.log(
				'Selected ingestion plugin:',
				selectedIngestionPlugin?.name,
				'with params:',
				ingestionPluginParams
			);
		}
	...
	async function runBaseIngestion() {
		...
		try {
			...
			const result = await runBaseIngestionPlugin(
				kbId,
				selectedIngestionPlugin.name,
				ingestionPluginParams
			);
...
<div class="overflow-hidden bg-white shadow sm:rounded-lg">
				...
				{#if serverOffline}
					<div>
						<p class="mb-2 font-bold">
							{$_('knowledgeBases.detail.serverOfflineTitle', {
								default: 'Knowledge Base Server Offline'
							})}
						</p>
						<p>
							{$_('knowledgeBases.detail.serverOfflineMessage', {
								default:
									'Could not load details. Please try again later or contact an administrator.'
							})}
						</p>
					</div>
				{:else}
	...
	{:else if kb}
		<div
			style="background-color: blue; color: white; padding: 10px; margin: 5px; font-weight: bold;"
		>
		...
		<div>
			...
			<div class="border-b border-gray-200">
				<dl>
					...
					{#if kb.owner}
						<!-- Keep check, assumes owner might exist -->
					...
					{#if kb.created_at}
						<!-- Keep check, assumes created_at might exist -->
						<div class="bg-gray-50 px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
							<dt class="text-sm font-medium text-gray-500">
			...
			<div
				style="background-color: green; color: white; padding: 10px; margin: 5px; font-weight: bold;"
			>
			...
			<div class="border-t border-gray-200">
				...
				<div class="px-4 py-5 sm:px-6">
					<p style="color: magenta; font-weight: bold; padding: 5px; border: 1px solid magenta;">
						DEBUG: Current activeTab is: {activeTab}
					</p>
					<!-- Files Panel -->
					{#if activeTab === 'files'}
						<div>
													...
													<td class="px-6 py-4 whitespace-nowrap">
														<div class="flex items-center">
															<div class="text-sm font-medium text-gray-900">
																{#if file.file_url}
																	<!-- Use updated KnowledgeBaseFile def -->
																	<a
																		href={file.file_url}
																		target="_blank"
					...
					{#if activeTab === 'ingest'}
						...
						{@const acceptTypes = isFilePlugin
							? (selectedIngestionPlugin?.supported_file_types ?? []).join(',')
							: ''}

						<!-- MORE DEBUGGING -->
						<div class="my-1 border border-blue-500 p-2">
							<p class="text-xs text-blue-700">
								SIMPLE DEBUG: Selected Plugin Name: {selectedIngestionPlugin?.name || 'Not Set'}
							</p>
						</div>
						...
						<div class="-mx-4 -my-5 bg-gray-50 px-4 py-5 sm:-mx-6 sm:px-6">
							<h4 class="text-md mb-4 font-medium text-gray-700">
								{isFilePlugin
									? $_('knowledgeBases.fileUpload.sectionTitleFile', {
											default: 'Upload and Ingest New File'
										})
									: $_('knowledgeBases.fileUpload.sectionTitleBase', {
											default: 'Configure and Run Ingestion'
										})}
							</h4>
							{#if loadingIngestionPlugins}
								<div class="py-4 text-center">
									<div class="animate-pulse text-gray-500">
										{$_('knowledgeBases.fileUpload.loadingPlugins', {
											default: 'Loading ingestion plugins...'
										})}
									</div>
								</div>
								...
								<div class="py-4 text-center">
									<div class="text-red-500">
										{#if serverOffline}
											<div>
												<p class="mb-2 font-bold">
													{$_('knowledgeBases.fileUpload.serverOffline', {
														default: 'Knowledge Base Server Offline'
													})}
												</p>
												<p>
													{$_('knowledgeBases.fileUpload.tryAgainLater', {
									...
									<button
										onclick={fetchIngestionPlugins}
										class="mt-4 inline-flex items-center rounded-md border border-transparent bg-[#2271b3] px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-[#195a91] focus:ring-2 focus:ring-[#2271b3] focus:ring-offset-2 focus:outline-none"
										style="background-color: #2271b3;"
									>
										{$_('knowledgeBases.fileUpload.retry', { default: 'Retry' })}
									</button>
								</div>
							{:else if ingestionPlugins.length === 0}
								<div class="py-4 text-center">
									<div class="text-gray-500">
										{$_('knowledgeBases.fileUpload.noPlugins', {
											default: 'No ingestion plugins available.'
										})}
									</div>
								</div>
							{:else}
								...
								>
									...
									{#if isFilePlugin}
										...
										<div>
											...
											>
												...
												{#if acceptTypes}<span class="ml-1 text-xs text-gray-500"
														>(Supported: {acceptTypes})</span
													>{/if}
											</label>
											...
											<select
												...
												onchange={(e) => {
													...
													if (select && select.value) {
														selectIngestionPlugin(parseInt(select.value));
													}
												}}
											>
												{#each ingestionPlugins as plugin, i}
													<option value={i} selected={i === selectedIngestionPluginIndex}>
									...
									<div class="my-2 border border-dashed border-red-500 p-4">
										<h6 class="font-bold text-red-700">DEBUGGING OUTPUT:</h6>
										<p class="text-xs text-gray-600">
											Selected Plugin Parameters (raw from template):
										</p>
										<pre class="overflow-x-auto rounded bg-red-50 p-2 text-xs">{JSON.stringify(
												selectedIngestionPlugin?.parameters,
												null,
												2
											)}</pre>
										<p class="mt-1 text-xs text-gray-600">
											Number of keys in parameters (template): {Object.keys(
												selectedIngestionPlugin?.parameters || {}
											).length}
										</p>
										<p class="mt-1 text-xs text-gray-600">
											Condition Check (template): {selectedIngestionPlugin &&
												selectedIngestionPlugin.parameters &&
												Object.keys(selectedIngestionPlugin.parameters).length > 0}
										</p>
									</div>

									{#if selectedIngestionPlugin && selectedIngestionPlugin.parameters && Object.keys(selectedIngestionPlugin.parameters).length > 0}
										<div class="space-y-4 border-t border-gray-200 pt-4">
											...
											{#each Object.entries(selectedIngestionPlugin.parameters) as [paramName, paramDef] (paramName)}
												{console.log('DEBUG: paramDef for', paramName, 'is:', paramDef)}
												<div>
													<label
														for={`param-${paramName}-ingest`}
														class="block text-sm font-medium text-gray-700"
													>
														{paramName}
														{paramDef.required ? ' *' : ''}
													...
													<div class="mt-1">
														{#if paramDef.enum && Array.isArray(paramDef.enum)}
															...
															>
																{#each paramDef.enum as enumValue}
																	<option
																		value={enumValue}
																		selected={ingestionPluginParams[paramName] === enumValue}
																	>
																		{enumValue}
																	</option>
																{/each}
														...
														{:else if paramDef.type === 'array'}
															<textarea
																...
																rows="3"
																class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2271b3] focus:ring-[#2271b3] sm:text-sm"
																value={Array.isArray(ingestionPluginParams[paramName])
																	? ingestionPluginParams[paramName].join('\n')
									...
									<div class="flex justify-end pt-4">
										<button
											...
											style="background-color: #2271b3;"
											disabled={!selectedIngestionPlugin ||
												uploading ||
												(isFilePlugin && !selectedFile)}
										>
											{#if uploading}
												<svg
			...
			<!-- End Files / Ingestion / Query Section -->
		</div>
	{:else}
